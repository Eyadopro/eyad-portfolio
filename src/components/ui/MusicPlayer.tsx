...
import { Play, Pause, SkipBack, SkipForward, Volume2, VolumeX, Minimize2, X, Menu } from 'lucide-react';
...
export default function MusicPlayer() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved) {
        ...
        if (state.track !== undefined) setCurrentTrack(state.track);
        if (state.size) setPlayerSize(state.size);
  ...
  useEffect(() => {
    ...
    if (isPlaying) {
      ...
      if (playPromise !== undefined) {
        playPromise.catch(() => console.log('Playback blocked'));
      }
    }
  ...
  const togglePlay = () => {
    ...
    if (isPlaying) {
      ...
      const playPromise = audio.play();
      if (playPromise !== undefined) {
        playPromise.then(() => setIsPlaying(true)).catch(() => console.log('Playback blocked'));
      }
    }
  };
      ...
      <AnimatePresence>
        {isVisible && (
          <motion.div
            ...
            dragMomentum={false}
            dragElastic={0}
            style={{ x, y, width: isExpanded ? currentSize.width : 64 }}
                ...
                >
                  ...
                  <div className="absolute top-2 left-2 right-2 flex justify-between items-center">
                    <div className="flex gap-1">
                      {(['small', 'medium', 'large'] as PlayerSize[]).map((size) => (
                        <motion.button
                          key={size}
                ...
                <div className="p-3">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                      <motion.button whileHover={{ scale: 1.2 }} whileTap={{ scale: 0.9 }} onClick={prevTrack} className="transition-transform">
                      ...
                      <motion.button whileHover={{ scale: 1.2 }} whileTap={{ scale: 0.9 }} onClick={nextTrack} className="transition-transform">
